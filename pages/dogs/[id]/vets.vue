<template>
      ...
      <div v-else-if="vets.length > 0" class="grid md:grid-cols-2 gap-6">
        ...
        >
          ...
          <div class="space-y-3 text-sm">
            ...
            <div v-if="vet.notes" class="flex items-start pt-2 border-t border-gray-200">
              <svg class="w-5 h-5 text-gray-400 mr-3 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
              </svg>
              <p class="text-gray-600 text-sm">{{ vet.notes }}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  <div v-if="showDeleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-lg max-w-md w-full p-6">
      <h3 class="text-xl font-bold text-gray-900 mb-4">Delete Veterinarian</h3>
      <p class="text-gray-600 mb-6">Are you sure you want to delete this vet? This action cannot be undone.</p>
...
usePageSeo(() => ({
  title: dogName.value ? `Vets — ${dogName.value}` : 'Dog Veterinarians',
  description: dogName.value
    ? `Manage veterinary contacts linked to ${dogName.value} on DogHealthy.`
    : 'Manage veterinary contacts for your dogs on DogHealthy.',
  index: false
}))
...
const fetchVets = async (isRefresh = false) => {
  try {
    if (isRefresh) {
...
const saveVet = async () => {
  try {
    ...
    // Then refresh the list (pass true to indicate this is a refresh)
    await fetchVets(true)
  } catch (err: any) {
